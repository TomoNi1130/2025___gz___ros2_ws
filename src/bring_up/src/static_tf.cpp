...
StaticTF::StaticTF(const rclcpp::NodeOptions &options) : rclcpp::Node("static_tf_node", options) {
  ...
  this->declare_parameter<std::string>("left_lidar_frame_id", "left_liadr");
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&StaticTF::msg_send, this));
}
...
void StaticTF::msg_send() {
  ...
  send_tf(odom_frame_id_, map_frame_id_, 0.0, 4.75, 0.0, 0.0, 0.0, 0.0);
}
