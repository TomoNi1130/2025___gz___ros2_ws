...
class RANSAC {
 public:
  RANSAC(const double threshold, const double gap_threshold, rclcpp::Logger logger) : max_iterations(100), distance_threshold(threshold), gap_threshold(gap_threshold), logger(logger) {}
      ...
      for (int line_num = 0; line_num < max_line_num; line_num++) {
        ...
        for (int j = 0; j < max_iterations; j++) {
          ...
          threads.push_back(std::thread(
              [this, guess_line_seg, &best_inlier_count, &return_line_seg, &guess_inliers]() { this->scrutinize_guess_line_seg(guess_line_seg, best_inlier_count, return_line_seg, guess_inliers); }));
        }
 ...
 private:
  ...
  void scrutinize_guess_line_seg(LineSeg guess_line_seg, int &best_inlier_count, LineSeg &return_line_seg, std::vector<bool> &guess_inliers) {
    ...
    if (guess_inlier_count != 0)  // 長さを出す（それに付随して、向き、初期位置の変更もある）
    {
      ...
      std::sort(distance_and_ids.begin(), distance_and_ids.end(), [](DistanceAndIDs &a, DistanceAndIDs &b) { return a.distance > b.distance; });  // 降順
      bool gap = false;
      ...
      if (distance_and_ids_from_termination_point.size() != 0) {
        std::sort(distance_and_ids_from_termination_point.begin(), distance_and_ids_from_termination_point.end(), [](DistanceAndIDs &a, DistanceAndIDs &b) { return a.distance > b.distance; });  // 降順
        gap = false;
